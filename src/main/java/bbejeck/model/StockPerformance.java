...
public class StockPerformance {
  ...
  public void updatePriceStats(double currentPrice) {
    ...
    currentAveragePrice =
        calculateNewAverage(currentPrice, currentAveragePrice, sharePriceLookback);
  }

  public void updateVolumeStats(int currentShareVolume) {
    this.currentShareVolume = currentShareVolume;
    shareDifferential =
        calculateDifferentialFromAverage((double) currentShareVolume, currentAverageVolume);
    currentAverageVolume =
        calculateNewAverage(currentShareVolume, currentAverageVolume, shareVolumeLookback);
  }
  ...
  private double calculateNewAverage(
      double newValue, double currentAverage, ArrayDeque<Double> deque) {
  ...
  public String toString() {
    return "StockPerformance{"
        ...
        + ", currentAveragePrice= "
        + decimalFormat.format(currentAveragePrice)
        + ", percentage difference= "
        + decimalFormat.format(priceDifferential)
        + ", currentShareVolume= "
        + decimalFormat.format(currentShareVolume)
        + ", currentAverageVolume= "
        + decimalFormat.format(currentAverageVolume)
        + ", shareDifferential= "
        + decimalFormat.format(shareDifferential)
        + '}';
  }
}
