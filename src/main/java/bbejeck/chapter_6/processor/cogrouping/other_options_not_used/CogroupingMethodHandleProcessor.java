...
// 这个 class 同 CogroupingProcessor 功能相同。但是，这里使用了一个 method 来代替 Punctuator。

/**
 * This class provides the same functionality as the {@link CogroupingProcessor} but the {@link
 * org.apache.kafka.streams.processor.Punctuator} is provided via a method handle versus a concrete
 * instance.
 */
public class CogroupingMethodHandleProcessor
  ...
  public void cogroup(long timestamp) {
    KeyValueIterator<String, Tuple<List<ClickEvent>, List<StockTransaction>>> iterator =
        tupleStore.all();

    while (iterator.hasNext()) {
      KeyValue<String, Tuple<List<ClickEvent>, List<StockTransaction>>> cogrouping =
          iterator.next();

      if (cogrouping.value != null
          && (!cogrouping.value._1.isEmpty() || !cogrouping.value._2.isEmpty())) {
