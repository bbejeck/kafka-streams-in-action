...
// 在 Transformer 中，维护 state store，以及 schedule 已定义的 Punctuator。
public class StockPerformanceStreamsAndProcessorApplication {
  ...
  public static void main(String[] args) throws Exception {
    ...
    builder.stream("stock-transactions", Consumed.with(stringSerde, stockTransactionSerde))
        .transform(
            () -> new StockPerformanceTransformer(stocksStateStore, differentialThreshold),
            stocksStateStore)   // Transformer 中维护 state store，以及 schedule Punctuator
    ...
    MockDataProducer.produceStockTransactionsWithKeyFunction(
        50, 50, 25, StockTransaction::getSymbol);
